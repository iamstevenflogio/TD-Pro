...
export default {
  ...
  actions: {
    async getRegisterData({ commit, dispatch }, payload) {
      const APIkey = "AIzaSyC4Nic5t4_K3NVSsldtLXnaBiby9Vyn5i0";
      const authUrl =
    ...
    async addNewUser({ commit, state }, payload) {
      try {
        const { data } = await axios.post(
          `https://vue-js-project-6b699-default-rtdb.firebaseio.com/user.json?auth=${state.token}`,
          payload
        );
    ...
    async getUser({ commit }, payload) {
      try {
        const { data } = await axios.get(
          `https://vue-js-project-6b699-default-rtdb.firebaseio.com/user.json`
        );
        for (let key in data) {
